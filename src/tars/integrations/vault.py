...
def _tars_root(cfg) -> Path:
    """All TARS-managed files live under vault/_TARS/ so the user's PARA
    structure (00_Inbox, 01_Projects, etc.) at the vault root stays clean.
    The leading-_ convention marks it as a system folder in Obsidian."""
    return _vault_root(cfg) / "_TARS"
...
def write_note(cfg, note_id: int, body: str, tags: Sequence[str] | None = None,
    ...
    notes_dir = _tars_root(cfg) / "notes"
...
def write_briefing(cfg, date_iso: str, summary: str) -> Path:
    """Write/overwrite a briefing file: vault/_TARS/briefings/YYYY-MM-DD.md"""
    path = _tars_root(cfg) / "briefings" / f"{date_iso}.md"
...
def write_followups(cfg, open_followups: list[dict]) -> Path:
    ...
    tz = ZoneInfo(getattr(cfg, "timezone", "UTC"))
    now = datetime.now(tz)
    path = _tars_root(cfg) / "follow-ups.md"
    front = _frontmatter({
