...
- **Phase 6 complete (MVP)** — AsyncIOScheduler with SQLAlchemyJobStore on same SQLite file. Job-runtime singleton holder pattern (jobs are pickled, runtime state isn't). Gmail/Calendar integrations (read-only, async-wrapped, refresh-token auto-renewal). 05:00 daily morning briefing live and ran end-to-end with real Gmail + Calendar + follow-ups in 10s for $0.0006. Polish round: human due times, empty-section stripping, format rules. Phase 6.1 (the other 4 scheduled jobs) deferred.
- **Phase 9b complete** — restic backups to Backblaze B2 working end-to-end. Daily 06:00 IDT via systemd timer (`tars-backup.timer`). Hot SQLite snapshot via Online Backup API (`.backup`), integrity-checked, included with `config.toml` + `google_token.json` + `client_secret.json` + `vault/`. Retention: 14 daily / 8 weekly / 12 monthly via `restic forget --prune`. Restore drill verified — `scripts/restore_from_backup.sh` pulls `latest` to `/tmp/tars-restore`, runs `PRAGMA integrity_check`, prints row counts. Storage Box (second destination) drops in as `RESTIC_REPOSITORY_2` later. Total recovery time from cold VPS: ~20 min.
- **Phase 6.1 complete** — email_summary every 30m (quiet 22:00-07:00, ingest both ways), calendar_pull every 15m, brain_reindex every 15m diff-mode (skip unchanged docs via body_hash), weekly_followup_reconcile Sun 18:00 with deterministic Telegram message format. All 5 jobs registered, picklable (parameter-free wrappers reading from scheduler.runtime singleton). 'tars job <name>' CLI subcommand for manual testing.
